  ...
  try {
    ...
    if (existing.rows.length > 0) {
      const updateRes = await query(
        ...
        [fullName.trim(), passwordHash, role || 'remote_sensing_analyst', organization || 'Independent / Research', normalizedEmail]
      );
      ...
      const insertRes = await query(
        ...
        [normalizedEmail, fullName.trim(), passwordHash, role || 'remote_sensing_analyst', organization || 'Independent / Research']
      );
